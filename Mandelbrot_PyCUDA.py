...
# Press 0 followed and then type a number in the terminal to change the power index of the iteration formula 
...
def key_selector(event):
    ...
    if event.key == u'left':  # Decrease  number of pixels
        ...
        ax.set_title('Side=%.2e, x=%.2e, y=%.2e, %s, L=%d'%(side,x0,y0,cmaps[i_cmap],L))
        plt.draw()
    if event.key=='0': # Change power index
        power = float(input("Type new power .. "))
        print("Power index set to %f" % power)
        i_cmap=49
        side=3.0; x0=-.5;y0=0.;L=400;
        func(np.float64(x0),np.float64(y0),np.float64(side), np.int32(L),np.float64(power),drv.Out(M),block=(n_block,n_block,1),grid=(n_grid,n_grid,1))            
        myobj = plt.imshow(M,cmap=cmaps[i_cmap],origin='lower')
        ax.set_title('Side=%.2e, x=%.2e, y=%.2e, %s, L=%d'%(side,x0,y0,cmaps[i_cmap],L))
        plt.draw()
...
def zoom_on_point(event):
    ...
    if not event.inaxes and event.x<.3*N : # Click on left side of image to reset to full fractal
        power=2.0; side=3.0; x0=-.5;y0=0.;i_cmap=49;L=400
        print("Power index set to %f" % power)
        func(np.float64(x0),np.float64(y0),np.float64(side), np.int32(L),np.float64(power),drv.Out(M),block=(n_block,n_block,1),grid=(n_grid,n_grid,1))        
        myobj = plt.imshow(M,cmap=cmaps[i_cmap],origin='lower')
        ax.set_title('Side=%.2e, x=%.2e, y=%.2e, %s, L=%d'%(side,x0,y0,cmaps[i_cmap],L))
